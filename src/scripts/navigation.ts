...
export class NavigationManager {
  ...
  private initElements(): void {
    ...
    this.searchTrigger = this.navSearch?.querySelector('.search-trigger') ?? null;
    this.searchPanel = this.navSearch?.querySelector('.nav-search-panel') ?? null;
    this.searchInput = this.navSearch?.querySelector<HTMLInputElement>('input') ?? null;
    this.menuToggleButton = document.getElementById('menu-toggle') as HTMLButtonElement | null;
  ...
  private bindEvents(): void {
    ...
    document.querySelectorAll('.search-close').forEach((button) => {
      button.addEventListener('click', (e) => {
        e.preventDefault();
        const mobileInput = (button.closest('form') as HTMLFormElement)?.querySelector<HTMLInputElement>('input');
        if (mobileInput) {
          mobileInput.value = '';
          mobileInput.focus();
        }
    ...
    document.addEventListener('click', (e) => {
      if (this.navSearch && e.target instanceof Node && !this.navSearch.contains(e.target)) {
        this.closeSearchPanel(false);
      }
    });
  ...
  public toggleSearchPanel(): void {
    ...
    this.searchTrigger?.setAttribute(NAV_CONFIG.ariaExpanded, String(nextState));
  ...
  public closeSearchPanel(clearInput = true): void {
    ...
    if (clearInput) {
      document.querySelectorAll<HTMLInputElement>('.nav-search input, .mobile-search input').forEach((input) => {
        input.value = '';
      });
    }
  }
  ...
  public toggleMobileNav(): void {
    ...
    this.menuToggleButton.setAttribute(NAV_CONFIG.ariaExpanded, String(!nextHidden));
    this.menuToggleButton.setAttribute(NAV_CONFIG.ariaLabel, nextHidden ? NAV_CONFIG.labels.openMenu : NAV_CONFIG.labels.closeMenu);
  }
  ...
  public closeMobileNav(): void {
    if (this.mobileNav && !this.mobileNav.hidden) {
      ...
      this.menuToggleButton?.setAttribute(NAV_CONFIG.ariaLabel, NAV_CONFIG.labels.openMenu);
    }
  }
}
